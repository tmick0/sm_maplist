...
public void OnPluginStart() {
    RegAdminCmd("sm_writemaplist", CmdWriteMapList, ADMFLAG_GENERIC);
...
int GenerateMapList(const char[] output) {
    char[] FilterFilePath = new char[PLATFORM_MAX_PATH];
    GetConVarString(CvarFilterFilePath, FilterFilePath, PLATFORM_MAX_PATH);

    char[][] tmp = new char[0][0]; // nullable arrays pls
    int filterc = LoadFilter(FilterFilePath, 0, tmp);
    char[][] filterv = new char[PLATFORM_MAX_PATH][filterc];
...
int FindMaps(Handle output, char[] directory, int depth, int filterc, const char[][] filterv) {
    ...
    char[] entry = new char[PLATFORM_MAX_PATH];
    FileType type;
    while (ReadDirEntry(dh, entry, PLATFORM_MAX_PATH, type)) {
        if (type == FileType_Directory) {
            char[] cpath = new char[PLATFORM_MAX_PATH];
